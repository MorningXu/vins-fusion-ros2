...
double last_vio_t = -1;
std::queue<sensor_msgs::msg::NavSatFix> gpsQueue;
std::mutex m_buf;
...
void GPS_callback(const sensor_msgs::msg::NavSatFix &GPS_msg) {
    //printf("gps_callback! \n");
    m_buf.lock();
...
void vio_callback(const nav_msgs::msg::Odometry &pose_msg) {
    ...
    while (!gpsQueue.empty()) {
        sensor_msgs::msg::NavSatFix GPS_msg = gpsQueue.front();
...
int main(int argc, char **argv) {
    ...
    auto n = std::make_shared<rclcpp::Node>("globalEstimator");

    global_path = globalEstimator.global_path;

    auto sub_GPS = n->create_subscription<sensor_msgs::msg::NavSatFix>("/gps", 100, std::bind(&GPS_callback,std::placeholders::_1));
    auto sub_vio = n->create_subscription<nav_msgs::msg::Odometry>("/vins_estimator/odometry", 100, std::bind(&vio_callback,std::placeholders::_1));
    pub_global_path = n->create_publisher<nav_msgs::msg::Path>("/globalEstimator/global_path", 100);
    pub_global_odometry = n->create_publisher<nav_msgs::msg::Odometry>("/globalEstimator/global_odometry", 100);
    pub_car = n->create_publisher<visualization_msgs::msg::MarkerArray>("/globalEstimator/car_model", 1000);
    rclcpp::spin(n);
    return 0;
}
