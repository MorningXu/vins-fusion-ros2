...
std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    TicToc t_r;
    cur_time = _cur_time;
    cur_img = _img;
    ...
    */
    ...
    if (prev_pts.size() > 0)
    {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"temporal optical flow costs: %fms", t_o.toc());
        //printf("track cnt %d\n", (int)ids.size());
    }
    ...
    {
        //rejectWithF();
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"set mask begins");
        TicToc t_m;
        setMask();
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"set mask costs %fms", t_m.toc());

        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"detect feature begins");
        TicToc t_t;
        ...
        if (n_max_cnt > 0)
        {
            ...
            n_pts.clear();
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"detect feature costs: %f ms", t_t.toc());
...
void FeatureTracker::rejectWithF()
{
    if (cur_pts.size() >= 8)
    {
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"FM ransac begins");
        TicToc t_f;
        ...
        reduceVector(ids, status);
        reduceVector(track_cnt, status);
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"FM ransac: %d -> %lu: %f", size_a, cur_pts.size(), 1.0 * cur_pts.size() / size_a);
        RCLCPP_DEBUG(rclcpp::get_logger("vins_estimator"),"FM ransac costs: %fms", t_f.toc());
    }
}
...
void FeatureTracker::readIntrinsicParameter(const std::vector<std::string> &calib_file)
{
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("vins_estimator"),"reading paramerter of camera %s", calib_file[i].c_str());
...
void FeatureTracker::showUndistortion(const std::string &name)
{
    ...
    for (int i = 0; i < int(undistortedp.size()); i++)
    {
        ...
        {
            //RCLCPP_ERROR("(%f %f) -> (%f %f)", distortedp[i].y, distortedp[i].x, pp.at<float>(1, 0), pp.at<float>(0, 0));
        }
    }
...
std::vector<cv::Point2f> FeatureTracker::undistortedPts(std::vector<cv::Point2f> &pts, camodocal::CameraPtr cam)
{
...
std::vector<cv::Point2f> FeatureTracker::ptsVelocity(std::vector<int> &ids, std::vector<cv::Point2f> &pts,
