...
void FeatureManager::initFramePoseByPnP(int frameCnt, Eigen::Vector3d Ps[], Eigen::Matrix3d Rs[], Eigen::Vector3d tic[], Eigen::Matrix3d ric[])
{
        ...
        for (auto &it_per_id : feature)
        {
            if (it_per_id.estimated_depth > 0)
            {
                ...
                if((int)it_per_id.feature_per_frame.size() >= index + 1)
                {
                    ...
                    Eigen::Vector3d ptsInWorld = Rs[it_per_id.start_frame] * ptsInCam + Ps[it_per_id.start_frame];
...
void FeatureManager::triangulate(int frameCnt, Eigen::Vector3d Ps[], Eigen::Matrix3d Rs[], Eigen::Vector3d tic[], Eigen::Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        if (it_per_id.estimated_depth > 0)
