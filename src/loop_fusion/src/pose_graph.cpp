...
void PoseGraph::registerPub(std::shared_ptr<rclcpp::Node> n)
{
    ...
    if (SHOW_L_EDGE)
    {
        if (cur_kf->has_loop)
        {
            ...
            if(cur_kf->sequence > 0)
            {
                ...
                posegraph_visualization->add_loopedge(P0, connected_P + Eigen::Vector3d(VISUALIZATION_SHIFT_X, VISUALIZATION_SHIFT_Y, 0));
            }
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                    ...
                    {
                        Eigen::Vector3d relative_t(t_array[i][0] - t_array[i-j][0], t_array[i][1] - t_array[i-j][1], t_array[i][2] - t_array[i-j][2]);
                        Eigen::Quaterniond q_i_j = Eigen::Quaterniond(q_array[i-j][0], q_array[i-j][1], q_array[i-j][2], q_array[i-j][3]);
                        Eigen::Quaterniond q_i = Eigen::Quaterniond(q_array[i][0], q_array[i][1], q_array[i][2], q_array[i][3]);
